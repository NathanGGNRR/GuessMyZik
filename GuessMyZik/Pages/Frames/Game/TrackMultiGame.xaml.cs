...
namespace GuessMyZik.Pages.Frames.Game
{
    ...
    /// </summary>
    public sealed partial class TrackMultiGame : Page
    {
        ...
        private int error = 0;
        private Player playerGuess;
        private List<Player> playerError = new List<Player>();
        private List<string> elementGuessed = new List<string>();

        public TrackMultiGame()
        {
        ...
        protected async override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            listViewPlayer.ItemsSource = parameter.players;
        ...
        private void CheckElementGuessed(string textTried, string element, string elementToFound, string textFound)
        {
            if (!elementGuessed.Contains(element))
            {
                if (textTried == elementToFound)
                {
                    for (int i = 0; i < parameter.textGuess.Count; i++)
                    {
                        if (parameter.textGuess[i].Tag.ToString() == element)
                        {
                            ...
                            textErrorPlayer.Visibility = Visibility.Collapsed;
                            textTry.IsEnabled = false;
                            parameter.players[parameter.players.IndexOf(playerGuess)].points += 1;
                            playerError.Clear();
                            Window.Current.CoreWindow.KeyUp += TrackGameMulti_KeyUp;
                            break;
                        }
                    }
        ...
        private void ErrorGame()
        {
            ...
            if(error == parameter.players.Count)
            {
                textErrorPlayer.Visibility = Visibility.Collapsed;
                playerError.Clear();
            }
        ...
        {
            int pointWinnerPlayer = 0;
            string playerWinner = "";
            foreach (Player player in parameter.players)
            {
                if (player.points > pointWinnerPlayer)
                {
                    pointWinnerPlayer = player.points;
        ...
        private async void TrackGameMulti_KeyUp(Windows.UI.Core.CoreWindow sender, Windows.UI.Core.KeyEventArgs e)
        {
            Window.Current.CoreWindow.KeyUp -= TrackGameMulti_KeyUp;
            foreach (Player player in parameter.players)
            {
                if(e.VirtualKey == player.key.Key && playerError.Contains(player) == false)
                {
                    textTry.IsEnabled = true;
                    textTry.Focus(FocusState.Keyboard);
                    playerGuesser.Visibility = Visibility.Visible;
                    playerGuess = player;
                    textGuesser.Text = player.name + " to guess";
                    if(playerError.Count > 0)
                    {
                        string allPlayerError = "";
                        foreach (Player playerError in playerError)
                        {
                            allPlayerError += playerError.name + " ";
                        }
                        textErrorPlayer.Visibility = Visibility.Visible;
                        textErrorPlayer.Text = allPlayerError + "can't play yet";
                    }
                    await Task.Delay(2000);
                    playerGuesser.Visibility = Visibility.Collapsed;
                    break;
                }
            }
            if(playerGuess == null)
        ...
        private void TrackGameMulti_Loaded(object sender, RoutedEventArgs e)
        {
            Window.Current.CoreWindow.KeyUp += TrackGameMulti_KeyUp;
        }
    }
